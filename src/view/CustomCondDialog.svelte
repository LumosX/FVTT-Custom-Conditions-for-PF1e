...
<script>
    ...
    const performItemFiltering = (item, customTagFilter) => 
        item.ownership[game.user.id] === 3 || item.system.tags?.map(x => x.toLowerCase()).includes(customTagFilter);

    const filteredConditions = writable([]);
    conditions.subscribe(value => {
        const filterTextbox = get(filterItems);
        if (!filterTextbox.active || !filterTextbox.value) {
            filteredConditions.set(value);
            return;
        }
        const customTagFilter = filterTextbox.value.trim().toLowerCase();
        filteredConditions.set(value.filter(item => performItemFiltering(item, customTagFilter)));
    });

    filterItems.subscribe(filterOptions => {
        const allConditions = get(conditions);
        if (!filterOptions.active || !filterOptions.value) {
            filteredConditions.set(allConditions);
            return;
        }
        const customTagFilter = filterOptions.value.trim().toLowerCase();
        filteredConditions.set(
            allConditions.filter(item => performItemFiltering(item, customTagFilter)),
        );
    });
    ...
    function parseValue(value, placeholder) {
        ...
        return isNaN(num) ? parseInt(get(placeholder)) : num;
    }
    ...
    filteredConditions.subscribe(fc => {
        statuses.subscribe(s => {
            ...
            allItems.set([...mappedConditions, ...mappedStatuses]);
        });
    });
</script>
